...
var frequencySort = function(s) {
  // n = s.length
  // m = num of distinct letters, is O(1) since there is at max 26 letters
  ...
  // O(m) => O(1)
  const maxHeap = new Heap(heapArr, (a, b) => a && b && a.freq > b.freq);

  let sortedS = '';

  // O(m * (logm + k)) => O(n)
  while (maxHeap.size()) {
    const nextMostFrequentChar = maxHeap.extract();

    for (let i = 0; i < nextMostFrequentChar.freq; i++) {
      sortedS += nextMostFrequentChar.char;
    }
  }
